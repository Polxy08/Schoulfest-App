...
def make_side_bracket(teams10):
    """
    Nebenfeld:
    - Runde 1: 6 Teams => 3 Matches
    - 4 Teams haben Freilos bis Runde 2
    - 1 Match in Runde 1 ist "Wildcard-Spiel": Gewinner bekommt in Runde 2 das Freilos
    """
    rnd = random.Random()
    t = teams10[:]
    rnd.shuffle(t)
    ...
    r1_play = t[:6]
    byes = t[6:]  # 4 teams frei in Runde 1

    r1 = [{"p1": r1_play[i], "p2": r1_play[i + 1], "winner": None} for i in range(0, 6, 2)]

    wildcard_match_index = rnd.randrange(0, 3)  # 0..2 -> welches R1 Match ist Wildcard-Spiel

    return {
        "meta": {
            "byes_round1": byes,                 # <<< WIRD ANGEZEIGT
            "wildcard_match_index": wildcard_match_index,  # <<< WIRD MARKIERT
        },
        "rounds": [
            {"name": "Runde 1 (6 Teams)", "matches": r1},
            {"name": "Runde 2 (7 Teams)", "matches": [{"p1": None, "p2": None, "winner": None} for _ in range(3)], "bye": None},
            {"name": "Halbfinale", "matches": [{"p1": None, "p2": None, "winner": None} for _ in range(2)]},
            {"name": "Finale", "matches": [{"p1": None, "p2": None, "winner": None}]},
        ],
    }
...
def advance_side(bracket):
    ...
    # Runde 1: wenn alle 3 Sieger da sind -> Runde 2 Teams setzen
    r1_winners = [m["winner"] for m in r[0]["matches"]]
    if all(r1_winners):
        wc_idx = meta["wildcard_match_index"]
        wildcard_team = r[0]["matches"][wc_idx]["winner"]  # Gewinner des markierten Matches

        # Runde 2: 6 Teams spielen (4 byes + 2 winner ohne wildcard) und 1 Team hat Freilos (wildcard_team)
    ...
    # Runde 2: wenn alle Sieger da sind und bye gesetzt -> Halbfinale setzen
...
def render_bracket_tree(bracket, key_prefix, *,
                        show_info_extra=False,
                        wildcard_match_index=None,
                        byes_round1=None):
    ...
    # <<< Anzeige: Freilos-Teams aus Runde 1 (Nebenfeld)
    if byes_round1:
        st.info("**Bye Runde 1:**\n\n- " + "\n- ".join(byes_round1))
...
if mode == "Gruppenphase":
    ...
    if locked:
        st.warning("Gruppenphase ist gesperrt (K.O. bereits ausgelost)")

    yg_names = [yg["name"] for yg in state["yeargroups"]]
    yg_name = st.selectbox("Welcher Jahrgang?", yg_names)
...
# -----------------------------
    ...
    if field == "Hauptfeld":
        ...
        render_bracket_tree(
            bracket,
            key_prefix="side",
            show_info_extra=True,
            wildcard_match_index=wildcard_match_index,
            byes_round1=byes_round1,
        )

        champ = bracket["rounds"][-1]["matches"][0]["winner"]
        if champ:
            st.success(f"🏆 Sieger Nebenfeld: **{champ}**")", "D", "G"]},
        {"name": "3e", "classes": ["BC", "D", "G"]},
    ],
    "points": {"win": 2, "draw": 1, "loss": 0},
}
